                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        {features.map((item) => (
                            <div
                                ...
                                className="bg-white p-10 rounded-lg shadow-sm hover:shadow-2xl transition-all duration-300 group cursor-pointer border border-gray-100 h-full flex flex-col items-start"
                            >
                                ...
                                <div className="flex items-center mt-auto">
                                    <span
                                        onClick={() => setSelectedFeature(item)}
                                        className="text-[#002b55] font-bold text-sm tracking-wider uppercase group-hover:text-[#2b7fff] group-hover:underline underline-offset-4 decoration-2 transition-all duration-300"
                                    >
                                        Read More
                                    </span>
                                </div>
                ...
                <div className="container mx-auto px-6 md:px-16 lg:px-24">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {services.map((service) => (
                            <div key={service.id} className="relative h-[450px] group overflow-hidden bg-gray-100 rounded-lg shadow-sm">
                                {/* Image */}
                                <img
                                    src={service.image}
                                    alt={service.title}
                                ...
                                {/* Overlay / Card */}
                                <div className="absolute bottom-6 left-6 right-8 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out z-10">
                                    <div className="bg-white p-6 shadow-xl relative rounded-sm">
                                        {/* Plus Icon - Opens Modal */}
                                        <button
                                            onClick={() => setSelectedService(service)}
                                            className="absolute -top-10 right-0 w-10 h-10 bg-[#f0b100] text-white flex items-center justify-center cursor-pointer hover:bg-[#2b7fff] transition-colors shadow-md"
                                        >
                                        ...
                                        <div className="flex items-center gap-2 mb-2">
                                            <div className="w-6 h-[2px] bg-[#f0b100]"></div>
                                            <span className="text-[#f0b100] font-medium text-sm uppercase">{service.category}</span>
                                        </div>
                                        <h3 className="text-xl font-bold text-[#002b55] leading-tight">
                                            {service.title}
                                        </h3>
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {team.map((member) => (
                            <div key={member.id} className="group relative">
                                ...
                                {/* Content Box - Overlapping Bottom */}
                                <div className="absolute bottom-6 left-6 right-6 bg-white p-8 shadow-xl rounded-sm text-center">
                                    <h4 className="text-[#f0b100] font-bold uppercase text-sm tracking-wider mb-2">{member.role}</h4>
                                    <h3 className="text-2xl font-bold text-[#002b55] mb-4">{member.name}</h3>
